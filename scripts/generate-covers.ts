...
for (const [index, article] of articlesSeed.entries()) {
  ...
  const file = join(OUT_DIR, `${article.slug}.jpg`);
  const info = await sharp(svg).jpeg({ quality: 72, mozjpeg: true }).toFile(file);
  total += info.size;
  console.log(`✓ ${article.slug}.jpg  ${(info.size / 1024).toFixed(0)} KB`);
}

console.log(`\nГотово: ${articlesSeed.length} обложек, ${(total / 1024).toFixed(0)} KB`);
